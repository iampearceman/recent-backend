...
import type { SyncFailure, SyncLog, DbSyncLogRow } from '../../core/sync-logs/model'
import type { SyncLogsRepository, ListSyncLogsByToolInput, ListRecentFailuresInput } from '../../core/sync-logs/repository'
...
export class DrizzleSyncLogsRepository implements SyncLogsRepository {
  ...
  async listByTool({ toolId, limit = DEFAULT_LIMIT, offset = 0 }: ListSyncLogsByToolInput): Promise<SyncLog[]> {
    try {
  ...
  async listRecentFailures({ limit = DEFAULT_LIMIT }: ListRecentFailuresInput = {}): Promise<SyncFailure[]> {
    try {
      const rows = await this.db
        .select()
        .from(syncLogs)
