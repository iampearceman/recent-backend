...
class MockDbClient {
	...
	limit() {
		...
		// We return the native Promise and attach an `offset` helper so callers can
		// either await the promise or call `.offset()` — avoid defining a `then`
		// property on a plain object; use the Promise instance instead.
		const p = Promise.resolve(this.rows) as any
		p.offset = () => Promise.resolve(this.rows)
		return p
	}
	offset() {
